...
def expectedReturn(state, action, stateValue):
    ...
    for rentalsLoc1 in range(0, POISSON_UPPER_BOUND):
        for rentalsLoc2 in range(0, POISSON_UPPER_BOUND):
            ...
            rentalsProb = poisson(rentalsLoc1, EXPECTED_FIRST_LOC_REQUESTS) * \
                poisson(rentalsLoc2, EXPECTED_SECOND_LOC_REQUESTS)
            # Total Rentals
            ...
            for returnsLoc1 in range(0, POISSON_UPPER_BOUND):
                for returnsLoc2 in range(0, POISSON_UPPER_BOUND):
                    ...
                    prob = poisson(returnsLoc1, EXPECTED_FIRST_LOC_RETURNS) * \
                        poisson(returnsLoc2, EXPECTED_SECOND_LOC_RETURNS) * rentalsProb
                    ...
                    # Number of cars at the end of the day
                    returns += prob * (rewards + DISCOUNT_RATE *
                                       stateValue[carsLoc1_prime, carsLoc2_prime])
    return returns
...
newStateVal = np.copy(stateVal)
policy = np.zeros(newStateVal.shape, dtype=np.int)
improvePolicy = False
for i, j in states:
...
if improvePolicy:
    ...
    for i, j in states:  # (i for the number of cars in place 1 and j for place 2)
        actionReturns = []
        for action in actions:
            if ((action >= 0 and i >= action) or (action < 0 and j >= np.absolute(action))):
                            # move car out action in place 1 is positive and for place 2 is negative
